...
这是一个**极其轻量级**的**B站客户端**，名字来源于原神中的“虚空终端”。本项目是 [哔哩终端 BiliClient](https://gitee.com/RobinNotBad/BiliClient) 停更后的改版分支，基于原版 2.7.0 开发，在保留原版轻量、流畅、低配设备友好的特性的同时，对代码进行了**全面 Kotlin 重构**，并新增了大量功能。
...
- **虚拟合集**：播放本地缓存视频或收藏夹视频时，自动将同一文件夹/收藏夹的视频组成合集，支持自由切换与自动联播
...
- **下载引擎优化**：多线程分片完整性校验与单分片自动重试，动态分片
- **强制更新机制**：可真正退出应用，安装完成后按版本号自动解除拦截
- **文章详情完善**：修复文章误跳动态详情，专栏正文加载与点赞/投币/收藏/cv号/日期显示
- **设置重构**：统一渲染体系、新增设置项搜索、设置 key 集中管理、返回图标统一
