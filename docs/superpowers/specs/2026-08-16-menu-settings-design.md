...
- 移入未启用的手势 = **拖拽已启用项到未启用区域**（放弃原来的长按两次隐藏模式）。
...
- 新建 `adapter/MenuSettingAdapter.kt`，单 adapter 多 view 类型，从上到下依次：
  ...
  3. "未启用"分区标题；
  4. 未启用项（灰色弱化显示）；
  5. 底部说明文字（普通灰色小字，内容：长按并拖动可调整顺序；拖入未启用区域可移入未启用；点击未启用项追加到已启用末尾；推荐、设置、缓存、搜索固定不可隐藏）。
- 交互：
  - 已启用项**长按一次** → ItemTouchHelper 开始拖拽，在已启用区内调整顺序。
  - 固定项（推荐/设置/缓存/搜索）**可参与排序**，但不可移入未启用区。
  - 已启用项**拖拽到未启用区域**（未启用标题或未启用项）→ 移入未启用区；固定项弹 toast"此项不可隐藏"。
  - **点击未启用项** → 追加到已启用列表末尾。
